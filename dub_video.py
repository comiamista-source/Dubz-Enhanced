...
try:
    from dub_engine import DubEngine, APIHealthError
...
def dub_chunk(item, src_lang, target_lang, genre, speakers, max_tries, progress):
    ...
    for attempt in range(1, max_tries + 1):
        try:
            ...
            # API health errors are critical and won't be fixed by retrying
            log(f"chunk {item['i']+1} CRITICAL API ERROR: {_scrub(str(e))[:80]}",
                icon=G_FAIL, color=C.RED)
            progress[item["i"]] = f"FAILED (API): {_scrub(str(e)[:30])}"
            return False
        except Exception as e:
            last = e
...
def main():
    ...
    # --- Health check: verify the API is accessible before starting ---
    log("Checking API health...", icon=G_WAIT, color=C.DIM)
    try:
        test_dub = DubEngine(poll_interval=_POLL_INTERVAL)
        test_dub.health_check()
        log("API is accessible and ready.", icon=G_OK, color=C.GRN)
    except APIHealthError as e:
        log(f"CRITICAL: API health check failed!", icon=G_FAIL, color=C.RED)
        log(f"Error: {_scrub(str(e))}", icon="!", color=C.RED)
        log(f"The free API endpoint may have been closed or is unreachable.",
            icon="!", color=C.RED)
        log(f"See: FRAGILITY_NOTE.md for details.", icon="!", color=C.RED)
        sys.exit(1)
    print()
